...
deny_reason := "daily_cap_exceeded" if { not frozen; input.amount_paise <= input.per_tx_cap_paise; (input.daily_spent_paise + input.amount_paise) > input.daily_cap_paise }
deny_reason := "payee_inactive" if { not frozen; input.amount_paise <= input.per_tx_cap_paise; (input.daily_spent_paise + input.amount_paise) <= input.daily_cap_paise; input.payee_active == false }
